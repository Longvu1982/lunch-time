            ...
            <div className="flex flex-wrap gap-3">
                {userGroups.map((group) => (
                    ...
                    >
                        ...
                        <p className="text-center text-base font-semibold mb-3">{group.groupName}</p>
                        <p className="text-center text-xs font-semibold opacity-50">Ngày tạo: {group.createdAt?.toDate()?.toLocaleDateString()}</p>
                    </div>
                ))}
                {userGroups.length === 0 && <p>Chưa có nhóm để hiển thị</p>}
            </div>
        </>
    );
};
